...
async fn extract_from_local_file(wheel_path: &str, all_metadata: bool) -> Result<()> {
    println!("Reading local wheel file: {}", wheel_path);
    ...
    println!("Found dist-info directory: {}", dist_info_dir);
    ...
    let has_metadata = archive
        ...
        .map(|file| {
            println!("Found {} ({} bytes)", BUILD_ENV_FILENAME, file.size());
            true
        })
        .unwrap_or(false);
    ...
    if all_metadata {
        ...
        let has_pkg_metadata = archive
            ...
            .map(|file| {
                println!("Found METADATA ({} bytes)", file.size());
                true
            })
            .unwrap_or(false);
...
async fn extract_from_cloud(uri: &str, all_metadata: bool) -> Result<()> {
    ...
    println!("Connecting to GCS bucket: {}", bucket);
...
impl GcsRangedReader {
    ...
    async fn new(client: Client, bucket: String, object: String) -> Result<Self> {
        ...
        println!("Object size: {} bytes", total_size);
    ...
    fn fetch_range(&self, start: u64, end: u64) -> BoxFuture<'static, std::io::Result<Bytes>> {
    ...
    fn get_data(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        let future = self.fetch_range(start, end);
        let data = match Handle::current().block_on(future) {
            Ok(data) => data,
            Err(e) => return Err(e),
        };
