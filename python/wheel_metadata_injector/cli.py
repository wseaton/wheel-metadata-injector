...
def main():
    ...
    parser.add_argument(
        "-e", "--env-file", help="Path to file containing list of environment variables to collect"
    )
    parser.add_argument(
        "-v", "--env-vars", help="Comma-separated list of environment variables to collect"
    )
    ...
    try:
        if env_vars_list:
            output_path = process_wheel_with_env_vars(wheel_path, env_vars_list, output_path)
        elif env_file:
